...
export default async function DashboardPage() {
  ...
  return (
    <Container className="space-y-5 pb-20">
      ...
      {/* v5 — Section Parcours : CECRL + Métier (brief V5 §6) */}
      <div>
        <h2 className="text-xs uppercase font-bold text-gray-500 mb-2">Tes parcours</h2>
        <div className="space-y-2">
          <Link href={"/session" as any}>
            <div className="p-3 rounded-xl border border-rule bg-white hover:border-primary-300 flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-primary-50 flex items-center justify-center text-2xl">📘</div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-sm text-primary-900">Parcours CECRL {lang?.cefr_global || 'A1'}</div>
                <div className="text-xs text-gray-600">Vocabulaire général par niveau européen</div>
              </div>
              <div className="text-primary-500 text-lg">→</div>
            </div>
          </Link>
          <Link href={"/grc" as any}>
            <div className="p-3 rounded-xl border border-rule bg-white hover:border-primary-300 flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-primary-50 flex items-center justify-center text-2xl">💼</div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-sm text-primary-900">Parcours Métier — GRC</div>
                <div className="text-xs text-gray-600">Gouvernance, Risques & Conformité (anglais pro)</div>
              </div>
              <div className="text-primary-500 text-lg">→</div>
            </div>
          </Link>
          <Link href={"/grammaire" as any}>
            <div className="p-3 rounded-xl border border-rule bg-white hover:border-primary-300 flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-primary-50 flex items-center justify-center text-2xl">📚</div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-sm text-primary-900">Grammaire</div>
                <div className="text-xs text-gray-600">279 leçons + exercices (A1 à C2)</div>
              </div>
              <div className="text-primary-500 text-lg">→</div>
            </div>
          </Link>
          {/* v9.1 — Nouveau parcours vocabulaire structuré (22 blocs A1, 7 phases par leçon) */}
          <Link href={"/vocab" as any}>
            <div className="p-3 rounded-xl border border-rule bg-white hover:border-primary-300 flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-primary-50 flex items-center justify-center text-2xl">🗣️</div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-sm text-primary-900">Vocabulaire A1 (nouveau)</div>
                <div className="text-xs text-gray-600">22 blocs thématiques · 121 leçons · 7 phases</div>
              </div>
              <div className="text-primary-500 text-lg">→</div>
            </div>
          </Link>
