  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-rule rounded-2xl p-6 text-center">
        ...
        <div className="text-2xl font-display font-extrabold text-primary-900">{w.lemma}</div>
          ...
          <div className="flex-1">
            ...
            {!feedback.ok && (
              <div className="text-sm text-gray-700 mt-1">
                La bonne réponse : <b className="text-ok">{w.translation || w.lemma}</b>
                <button onClick={() => speak(w.translation || w.lemma, voiceName)} className="ml-2 text-xs bg-white border border-rule px-2 py-0.5 rounded-full">🔊</button>
              </div>
            )}
          </div>
        </div>
