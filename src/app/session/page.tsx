/**
 * v9.0.4 — Base v3.7 (production GitHub) + 3 enrichissements alignés grammaire :
 *   (A) Auto-voice EN → FR au montage de DiscoveryPhase via speakSequence.
 *   (B) Lecture vocale du pavé final ("Bravo {prénom}" + liste mots) sur l'écran done.
 *   (C) Footer "TTS vX.Y" pour vérifier en un coup d'œil quel build est actif.
 *
 * Tout le reste (Mascot, qcm, cloze, flashcard, correction_review, localStorage reprise,
 * tour de remédiation, mode révision) est conservé tel quel.
 *
...
import {
  speak, getBestVoice, waitForVoices,
  speakSequence, stopSpeaking, TTS_VERSION, type SequenceSegment,
...
export default function SessionRunner() {
  ...
  // v9.0.4 — Prénom utilisateur pour "Bravo {prénom}" lu à voix haute
  ...
  // v9.0.4 — Track des lemmas découverts pour le pavé final lu
  const seenLemmas = useRef<string[]>([])

  const current = plan[idx]
  const word = current ? words[current.word_id] : null

  // v9.0.4 — Tracker chaque mot rencontré (pour la lecture finale)
  useEffect(() => {
    if (word?.lemma && !seenLemmas.current.includes(word.lemma)) {
      seenLemmas.current.push(word.lemma)
    }
  }, [word?.lemma])
  ...
  useEffect(() => {
    (async () => {
      ...
      const storageKey = `dodolingua-session-v2-${isReview ? 'rev' : courseId ? `course-${courseId}` : 'learn'}-${typeFilter || 'def'}`
      if (!forceNew) {
        try {
          ...
          if (saved) {
            ...
            if (ageH < 24 && state.plan?.length > 0 && state.idx < state.plan.length) {
              ...
              if (user) {
                ...
                // v9.0.4 — charge le prénom aussi en reprise
      ...
      if (user) {
        ...
        // v9.0.4 — Prénom pour la lecture vocale finale
        const { data: prof } = await supabase.from('profiles')
          .select('display_name').eq('id', user.id).maybeSingle()
        if (prof?.display_name) setUserName(prof.display_name)
      }
    })()
    // v9.0.4 — Stop TTS si on quitte la page
    return () => { stopSpeaking() }
  }, [])
  ...
  /* ---------- v9.0.4 — Lecture vocale du pavé final ---------- */
  ...
  useEffect(() => {
    if (done && finalSegments.length > 0) {
      const t = setTimeout(() => speakSequence(finalSegments, 0.9), 600)
  ...
  if (done) {
    ...
    const totalAnswered = evalResults.length
    ...
    // v9.0.4 — Personnalise le titre avec le prénom
    const firstName = userName?.split(' ')[0]
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6">
        <Card className="max-w-md text-center space-y-5 overflow-visible">
          ...
          <div className="-mt-2">
            <h1 className="text-3xl font-extrabold text-primary-900 animate-pop-in">
              Bravo {firstName || ''} !
            </h1>
            <div className="text-base text-primary-700 mt-1">{dodoMessage}</div>
          ...
          {/* v9.0.4 — Pavé vert mots vus (lu à voix haute) */}
          ...
          {/* v9.0.4 — Footer version (cohérent grammaire) */}
    ...
    <main className="min-h-screen flex items-start justify-center p-4 pb-20">
      <Container className="max-w-md space-y-3">
        ...
        <div className="flex items-center justify-center gap-2 text-[10px] text-gray-400 select-none pt-1">
          <span data-tts-version={TTS_VERSION}>TTS {TTS_VERSION}</span>
          <span>·</span>
          <span>Vocabulaire — {current.phase}</span>
        </div>
      </Container>
    </main>
  )
...
function DiscoveryPhase({ word, voiceName, onNext, busy }: { word: WordData; voiceName: string | null; onNext: () => void; busy: boolean }) {
  // v9.0.4 — Auto-voice EN → FR au montage de la phase Discovery.
  // Daniel lit le mot EN, puis Thomas dit "veut dire [traduction]",
  // puis l'exemple EN (s'il existe), puis la traduction française de l'exemple.
  useEffect(() => {
    const segs: SequenceSegment[] = []
    segs.push({ text: word.lemma, lang: 'en-GB', pauseAfter: 600 })
    if (word.gloss_fr) {
      segs.push({ text: `veut dire ${word.gloss_fr}`, lang: 'fr-FR', pauseAfter: 800 })
    }
    if (word.example) {
      segs.push({ text: 'Par exemple…', lang: 'fr-FR', pauseAfter: 400 })
      segs.push({ text: word.example, lang: 'en-GB', pauseAfter: 600 })
    }
    const t = setTimeout(() => speakSequence(segs, 0.9), 600)
    return () => { clearTimeout(t); stopSpeaking() }
  }, [word.lemma, word.gloss_fr, word.example])
