...
export default function GamePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const w: GameWord[] = (cs || []).map((c: any) => ({
        ...
        translation: c.gloss_fr,
        lemma: c.translations?.[0]?.lemma || '', ipa: c.translations?.[0]?.ipa,
        audio_url: c.translations?.[0]?.audio_url,
        example: c.translations?.[0]?.example,
        example_fr: c.translations?.[0]?.example_fr,
      }))
      setWords(w)
  ...
  // v5 — "Continuer plus tard" : retour au dashboard sans clore la quête.
  // La progression partielle est gardée côté UI/BDD selon les exercices déjà résolus.
  function handlePause(_results: any[]) {
